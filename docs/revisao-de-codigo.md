...
- **Repositório-raiz `claude-book.github.io` criado** (na noite de 08/07/2026): página-vitrine da organização (apresenta o livro e lista os repositórios) + cópia do mesmo arquivo de verificação. Motivo: o verificador de marca recusava a propriedade do subcaminho — pegadinha conhecida de sites `github.io` hospedados em "pasta" —, então registramos também a **raiz** `https://claude-book.github.io/` no Search Console. ⚠️ O arquivo `google610e27547249eb1a.html` na raiz desse repositório também **não pode ser removido**.
- **Status ao fim de 08/07:** as duas propriedades verificadas no Search Console e o nome do app aceito; o verificador de marca ainda respondia "not registered to you" — atribuído ao atraso de sincronização entre o Search Console e o sistema OAuth (horas, até ~48h). Próximo passo: repetir "I have fixed the issues" no dia seguinte; se persistir, plano C = adicionar a meta tag `google-site-verification` ao `docs/index.html` do projeto.

### 09/07/2026 — Desfecho da verificação da marca: inviável por política do Google (e está tudo bem)

**O que aconteceu:** a sincronização overnight não resolveu ("not registered to you" persistia). Executamos o plano C — a meta tag `google-site-verification` foi adicionada ao `<head>` das duas páginas (projeto e raiz da organização; ⚠️ **não remover**, são prova de propriedade). Em seguida, o plano D ("I believe the issues found are incorrect" → revisão adicional) levou ao questionário completo de verificação de apps — e ele revelou a resposta oficial.

**A descoberta (citação do próprio formulário do Google):** *"Production apps for personal use are not reviewed and will show the unverified app warning during sign-in."* — apps em produção de **uso pessoal não são revisados** e mantêm o aviso de "app não verificado". Como a arquitetura deste projeto determina que **cada leitor do livro cria o próprio projeto no Google Cloud** (o `client_secret` do autor nunca sai da máquina dele — está no `.gitignore`), este projeto OAuth (`claude-forms-mcp-501013`) tem exatamente **um usuário: o autor**. É, por definição, uso pessoal — e portanto **o selo de verificação não existe para este perfil de projeto**. O aviso amarelo no Branding não é um defeito: é o estado permanente e normal de um app pessoal em produção.

**Decisão:** o formulário de verificação **não foi enviado** (marcar "não é uso pessoal" para forçar a revisão seria faltar com a verdade, e o processo — vídeos de demonstração, justificativas de escopo, semanas de idas e vindas — não traria benefício algum). O aviso fica, aceito e compreendido.

**O que permanece de valioso (nada foi em vão):**
- As duas propriedades continuam verificadas no Search Console (raiz e projeto, por dois métodos cada: arquivo HTML + meta tag) — ⚠️ arquivos `google610e27547249eb1a.html` e meta tags devem permanecer nas páginas.
- Nome do app alinhado ("mcp-server-google-forms"), campos do Branding preenchidos e salvos, site do projeto e vitrine da organização no ar.
